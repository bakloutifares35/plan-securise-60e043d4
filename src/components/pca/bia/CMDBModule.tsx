...
import { 
  ...
  Check, AlertCircle as AlertCircleIcon
} from "lucide-react";
...
const TABS = [
  ...
  { id: 'equipment', label: 'Équipements', icon: Monitor },
  { id: 'app', label: 'Applications IT', icon: Server },
  { id: 'supplier', label: 'Prestataires', icon: Handshake },
];
...
// ============================================================
// COMPOSANT - KpiCard (avec hover vert)
// ============================================================
const KpiCard = ({
  label,
  value,
  ...
  return (
    <Card 
      className={cn(
        "border-[#E8E4DC] shadow-sm bg-white/60 transition-all duration-200",
        canClick && "cursor-pointer hover:border-[#2A5141] hover:bg-[#2A5141]/5",
        isActive && "border-[#2A5141] bg-[#2A5141]/10 shadow-sm",
        !canClick && "opacity-80"
      )}
    ...
    >
      <CardContent className="p-3">
        <div className="flex items-center justify-between">
          ...
          <div className="text-[#172030]/30">{icon}</div>
        </div>
        <div className="flex items-center gap-2 mt-1">
          <p 
            className={cn(
              "text-xl font-bold",
              isAlert ? "text-orange-500" : color || "text-[#172030]"
            )} 
        ...
        {isAlert && (
          <p className="text-[10px] text-orange-500/70 mt-0.5">{alertThreshold.message}</p>
        )}
      </CardContent>
    </Card>
  );
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="border border-[#E8E4DC] rounded-xl p-4 bg-white">
...
// ============================================================
// COMPOSANT - ResourceRow (sans colonne département, sans Dupliquer)
// ============================================================
const ResourceRow = ({ 
  resource, 
  type, 
  ...
  const getIcon = () => {
    switch(type) {
      ...
      case 'equipment': return <Monitor className="h-4 w-4 text-yellow-600" />;
  ...
  const getUsageBadge = () => {
    if (isUsed) {
      return (
        <Badge className="bg-green-50 text-green-700 border-green-200 hover:bg-green-50 cursor-pointer">
          {resource.used_by_count} processus
        </Badge>
      );
    } else {
      return (
        <Badge className="bg-orange-50 text-orange-700 border-orange-200 hover:bg-orange-50 cursor-pointer">
          Non utilisé
        </Badge>
      );
    }
  ...
  return (
    <TableRow 
      className={cn(
        "border-b border-[#E8E4DC] transition-all duration-200 cursor-pointer",
        index % 2 === 0 ? "bg-white" : "bg-[#FAFAF9]",
        isHovered && "shadow-[0_2px_8px_rgba(23,32,48,0.06)] border-l-2 border-l-[#2A5141]"
      )}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      onClick={() => onClick(resource)}
    >
      {/* Nom avec avatar/icône */}
      <TableCell className="py-3.5">
        <div className="flex items-center gap-3">
          {type === 'hr' ? (
            ...
            <div className="w-8 h-8 rounded-lg bg-[#F8F6F2] flex items-center justify-center border border-[#E8E4DC] flex-shrink-0">
              {getIcon()}
            </div>
          )}
          <div>
            <span className="font-medium text-[#172030]">{resource.name}</span>
            {hasCriticalImpact && (
              <Badge className="ml-2 bg-red-50 text-red-700 border-red-200 text-[10px] align-middle">
      ...
      {/* Champs spécifiques selon le type (sans département) */}
      {type === 'hr' && (
        <>
          <TableCell className="py-3.5 text-sm text-[#172030]/60">{resource.role || "—"}</TableCell>
          <TableCell className="py-3.5 text-sm text-[#172030]/60">{resource.email || "—"}</TableCell>
          <TableCell className="py-3.5 text-sm text-[#172030]/60">{resource.phone || "—"}</TableCell>
        </>
      )}
      {type === 'equipment' && (
        <>
          <TableCell className="py-3.5 text-sm text-[#172030]/60">{resource.type || "—"}</TableCell>
          <TableCell className="py-3.5 text-sm text-[#172030]/60">{resource.quantity || 1}</TableCell>
        </>
      )}
      {type === 'app' && (
        <TableCell className="py-3.5 text-sm text-[#172030]/60">{resource.remplacablepar || "—"}</TableCell>
      )}
      {type === 'supplier' && (
        <>
          <TableCell className="py-3.5 text-sm text-[#172030]/60">{resource.service || "—"}</TableCell>
          <TableCell className="py-3.5 text-sm text-[#172030]/60">{resource.contact || "—"}</TableCell>
        </>
      )}
      ...
      <TableCell className="py-3.5">
        <Popover open={showProcesses} onOpenChange={setShowProcesses}>
          ...
          <PopoverContent className="w-64 p-3 border-[#E8E4DC] shadow-lg" onClick={(e) => e.stopPropagation()}>
            <div className="space-y-2">
              <p className="text-xs font-medium text-[#172030]/60 uppercase tracking-wider">
                Processus liés ({getProcessNames().length})
              </p>
              {getProcessNames().length > 0 ? (
                <div className="space-y-1 max-h-40 overflow-y-auto">
                  {getProcessNames().map((name: string, i: number) => (
                    <div key={i} className="text-sm text-[#172030] py-1 border-b border-[#E8E4DC]/30 last:border-0">
                      {name}
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-[#172030]/40">Aucun processus lié</p>
              )}
            </div>
          </PopoverContent>
        </Popover>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0">
              <MoreVertical className="h-4 w-4 text-[#172030]/40" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="border-[#E8E4DC] shadow-lg">
            <DropdownMenuItem onClick={() => onClick(resource)} className="cursor-pointer">
              <Eye className="h-4 w-4 mr-2 text-[#172030]/60" />
              Voir le détail
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => onEdit(resource)} className="cursor-pointer">
              <Pencil className="h-4 w-4 mr-2 text-[#172030]/60" />
              Modifier
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => onViewProcesses(resource)} className="cursor-pointer">
              <Link className="h-4 w-4 mr-2 text-[#172030]/60" />
...
// ============================================================
// COMPOSANT - ResourceTable (sans colonne département)
// ============================================================
const ResourceTable = ({
  data,
  type,
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-[#F8F6F2] border-b border-[#E8E4DC]">
            {columns.map((col) => (
              <TableHead key={col.key} className="text-[10px] font-semibold text-[#172030]/50 uppercase tracking-wider py-3">
                {col.label}
              </TableHead>
            ))}
            <TableHead className="text-[10px] font-semibold text-[#172030]/50 uppercase tracking-wider py-3 text-center">
              Actions
            </TableHead>
          </TableRow>
        </TableHeader>
...
// ============================================================
// COMPOSANT - ResourceGridCard (sans département, sans Dupliquer)
// ============================================================
const ResourceGridCard = ({
  ...
  const getIcon = () => {
    switch(type) {
      ...
      case 'equipment': return <Monitor className="h-5 w-5 text-yellow-600" />;
  ...
  return (
    <div 
      className="border border-[#E8E4DC] rounded-xl p-4 bg-white hover:shadow-md hover:border-[#2A5141]/30 transition-all cursor-pointer"
      onClick={() => onClick(resource)}
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3 min-w-0">
          {type === 'hr' ? (
            <div className="w-10 h-10 rounded-full bg-[#2A5141]/10 flex items-center justify-center text-sm font-semibold text-[#2A5141] flex-shrink-0">
              {getInitials(resource.name)}
            </div>
          ) : (
            <div className="w-10 h-10 rounded-lg bg-[#F8F6F2] flex items-center justify-center border border-[#E8E4DC] flex-shrink-0">
              {getIcon()}
            </div>
          )}
          <div className="min-w-0">
            <p className="font-medium text-[#172030] truncate">{resource.name}</p>
            {hasCriticalImpact && (
              <Badge className="mt-0.5 bg-red-50 text-red-700 border-red-200 text-[10px]">
      ...
      <div className="mt-3 pt-3 border-t border-[#E8E4DC] flex items-center justify-between">
        <Badge 
          className={cn(
            "text-[10px]",
            isUsed ? "bg-green-50 text-green-700 border-green-200" : "bg-orange-50 text-orange-700 border-orange-200"
          )}
        >
...
// ============================================================
// COMPOSANT - ResourceDetailSheet (CORRIGÉ - SAUVEGARDE FONCTIONNELLE)
// ============================================================
const ResourceDetailSheet = ({
  ...
  // Mise à jour du formulaire quand la ressource change
  useEffect(() => {
    if (resource) {
      console.log('📝 Mise à jour du formulaire avec:', resource);
  ...
  const getIcon = () => {
    switch(type) {
      ...
      case 'equipment': return <Monitor className="h-12 w-12 text-yellow-600" />;
  ...
  const getCriticalityBadge = (criticality: Criticality) => {
    ...
    return colors[criticality] || 'bg-gray-100 text-gray-700 border-gray-200';
  };
  ...
  const handleSave = async () => {
    ...
    try {
      // Nettoyer les données : enlever les champs qui ne doivent pas être sauvegardés
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input 
                ...
                className={cn(
                  "mt-1 border-[#E8E4DC]",
                  formErrors.name && "border-red-400 focus:ring-red-400"
                )}
  ...
  return (
    <Sheet open={open} onOpenChange={(newOpen) => {
      ...
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        ...
        {resource && (
          <>
            <div className="flex items-center gap-4 py-4 border-b border-[#E8E4DC]">
              {type === 'hr' ? (
                <div className="w-16 h-16 rounded-full bg-[#2A5141]/10 flex items-center justify-center text-xl font-semibold text-[#2A5141]">
                  {getInitials(resource.name)}
                </div>
              ) : (
                <div className="w-16 h-16 rounded-xl bg-[#F8F6F2] flex items-center justify-center border border-[#E8E4DC]">
                  {getIcon()}
                </div>
              )}
              <div>
                <p className="text-lg font-semibold text-[#172030]">{resource.name}</p>
                <p className="text-sm text-[#172030]/50">{getTypeLabel()}</p>
            ...
            {isEditing ? (
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-1">
                  <p className="text-xs text-[#172030]/40 uppercase tracking-wider font-medium">Informations générales</p>
                  <div className="grid grid-cols-2 gap-2 bg-[#F8F6F2] rounded-lg p-3">
                    <div>
                      <p className="text-[10px] text-[#172030]/40">Nom</p>
                      <p className="text-sm font-medium">{resource.name}</p>
                    </div>
                    {type === 'hr' && (
                      <>
                        <div>
                          <p className="text-[10px] text-[#172030]/40">Rôle</p>
                          <p className="text-sm">{resource.role || "—"}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[#172030]/40">Email</p>
                          <p className="text-sm">{resource.email || "—"}</p>
                        </div>
                        <div className="col-span-2">
                          <p className="text-[10px] text-[#172030]/40">Téléphone</p>
                    ...
                    {type === 'equipment' && (
                      <>
                        <div>
                          <p className="text-[10px] text-[#172030]/40">Type</p>
                          <p className="text-sm">{resource.type || "—"}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[#172030]/40">Quantité</p>
                          <p className="text-sm">{resource.quantity || 1}</p>
                        </div>
                      </>
                    )}
                    {type === 'app' && (
                      <>
                        <div className="col-span-2">
                          <p className="text-[10px] text-[#172030]/40">Alternative</p>
                          <p className="text-sm">{resource.remplacablepar || "—"}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[#172030]/40">RTO</p>
                          <p className="text-sm">{resource.rto_hours || "—"}h</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[#172030]/40">RPO</p>
                          <p className="text-sm">{resource.rpo_hours || "—"}h</p>
                        </div>
                      </>
                    )}
                    {type === 'supplier' && (
                      <>
                        <div className="col-span-2">
                          <p className="text-[10px] text-[#172030]/40">Service</p>
                          <p className="text-sm">{resource.service || "—"}</p>
                        </div>
                        <div className="col-span-2">
                          <p className="text-[10px] text-[#172030]/40">Contact</p>
                ...
                {(resource.created_at || resource.updated_at) && (
                  <div className="space-y-1">
                    <p className="text-xs text-[#172030]/40 uppercase tracking-wider font-medium">Audit</p>
                    <div className="bg-[#F8F6F2] rounded-lg p-3 space-y-1">
                      {resource.created_at && (
                        <div className="flex items-center gap-2 text-sm">
                          <Calendar className="h-3.5 w-3.5 text-[#172030]/40" />
                          <span className="text-[#172030]/50">Créée le</span>
                          <span className="font-medium">{formatDate(resource.created_at)}</span>
                        </div>
                      )}
                      {resource.updated_at && (
                        <div className="flex items-center gap-2 text-sm">
                          <Clock className="h-3.5 w-3.5 text-[#172030]/40" />
                          <span className="text-[#172030]/50">Modifiée</span>
                          <span className="font-medium">{getTimeAgo(resource.updated_at)}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-1">
                  <p className="text-xs text-[#172030]/40 uppercase tracking-wider font-medium">
                  ...
                  {linkedProcesses.length > 0 ? (
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="bg-[#F8F6F2]">
                            <TableHead className="text-[10px] font-semibold text-[#172030]/50 py-2">Processus</TableHead>
                            <TableHead className="text-[10px] font-semibold text-[#172030]/50 py-2">Criticité</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {linkedProcesses.map((p) => {
                            ...
                            return (
                              <TableRow key={p.id} className="border-b border-[#E8E4DC]">
                                <TableCell className="py-2 text-sm">{p.name}</TableCell>
                                <TableCell className="py-2">
                                  <Badge className={getCriticalityBadge(crit)}>
                                    {crit}
                                  </Badge>
                                </TableCell>
                              </TableRow>
                    ...
                    <p className="text-sm text-[#172030]/40 italic">Aucun processus lié</p>
                  )}
                </div>
...
// ============================================================
// COMPOSANT - ImportExcelDialog (AVEC ÉDITION DES LIGNES)
// ============================================================
const ImportExcelDialog = ({
  ...
  const getFieldLabels = () => {
    ...
    const specific = {
      ...
      app: {
        remplacablepar: 'Application alternative',
    ...
    return { ...base, ...specific[resourceType] };
  };

  const generateTemplate = () => {
    const fieldLabels = getFieldLabels();
    const templateData = [Object.values(fieldLabels)];
    const exampleRow = Object.keys(fieldLabels).map((key) => {
      const examples: Record<string, string> = {
        ...
        type: 'Serveur',
        quantity: '2',
        remplacablepar: 'Alternative manuelle',
        rto_hours: '4',
        rpo_hours: '2',
        service: 'Hébergement cloud',
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Créer une copie éditable des données
        const editable = jsonData.map((row, index) => ({
          ...row,
          _index: index,
          _isNew: true,
        }));
  ...
  const saveEditRow = () => {
    ...
    // Mettre à jour aussi les données d'aperçu
    const previewIndex = editingRowIndex;
    if (previewIndex < previewData.length) {
      const newPreviewData = [...previewData];
      newPreviewData[previewIndex] = { ...editRowData };
  ...
  const getFieldKey = (header: string): string => {
    ...
    for (const fieldKey of fieldKeys) {
      const label = fieldLabels[fieldKey as keyof typeof fieldLabels].toLowerCase().replace(' *', '');
  ...
  const handleImport = async () => {
    ...
    for (const row of editableData) {
      ...
      for (const [header, fieldKey] of Object.entries(mapping)) {
        ...
        if (fieldKey === 'name') {
          ...
          if (!value) {
            hasError = true;
            errorMsg = `Ligne ${processed + 1}: Nom manquant`;
            break;
          }
          item.name = value;
        } else if (fieldKey === 'quantity' || fieldKey === 'rto_hours' || fieldKey === 'rpo_hours') {
          const val = row[header];
          item[fieldKey] = val !== undefined && val !== '' ? Number(val) : 0;
        } else {
          item[fieldKey] = String(row[header] || '').trim();
        }
      }
      ...
      if (hasError || !item.name) {
        results.errors++;
        if (errorMsg) results.errorDetails.push(errorMsg);
        continue;
      }
      ...
      if (existing) {
        try {
          ...
          results.errors++;
          results.errorDetails.push(`Erreur mise à jour "${item.name}"`);
        }
      } else {
        try {
          await supabase
          ...
          results.errors++;
          results.errorDetails.push(`Erreur création "${item.name}"`);
        }
      }
      
      setImportProgress(Math.min(processed, editableData.length));
    }
    ...
    if (results.created > 0 || results.updated > 0) {
      toast.success(`Import terminé: ${results.created} créées, ${results.updated} mises à jour`);
      onImportComplete();
    }
  };
    ...
    <Dialog open={open} onOpenChange={(newOpen) => {
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 1 ? 'Étape 1 : Sélectionnez votre fichier' : 'Étape 2 : Aperçu, modification et import'}
          </DialogDescription>
        </DialogHeader>
        ...
        {step === 1 && (
          <div className="space-y-4 py-4">
            <div 
              className="border-2 border-dashed border-[#E8E4DC] rounded-xl p-8 text-center hover:border-[#2A5141] transition-colors cursor-pointer"
            ...
            >
              ...
              />
              <Upload className="h-12 w-12 mx-auto text-[#172030]/20" />
              <p className="mt-4 text-sm text-[#172030]/60">
              ...
              {file && (
                <div className="mt-4 p-3 bg-[#F8F6F2] rounded-lg flex items-center gap-3">
                  <FileSpreadsheet className="h-5 w-5 text-[#2A5141]" />
              ...
              <>
                <div className="bg-[#F8F6F2] rounded-lg p-4 flex items-center justify-between">
                  <p className="text-sm text-[#172030]/60">
                    <span className="font-medium">{totalRows}</span> lignes prêtes à être importées
                  </p>
                ...
                <div className="border rounded-lg overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-[#F8F6F2]">
                        <TableHead className="text-[10px] font-semibold text-[#172030]/50 uppercase tracking-wider py-2 w-10">
                          #
                        </TableHead>
                        {headers.map((h) => (
                          <TableHead key={h} className="text-[10px] font-semibold text-[#172030]/50 uppercase tracking-wider py-2">
                            {h}
                          </TableHead>
                        ))}
                        <TableHead className="text-[10px] font-semibold text-[#172030]/50 uppercase tracking-wider py-2 text-center">
                          Actions
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {editableData.map((row, index) => {
                        ...
                        return (
                          ...
                          >
                            <TableCell className="py-2 text-center text-sm text-[#172030]/40">
                              {index + 1}
                            </TableCell>
                            {headers.map((header) => (
                              <TableCell key={header} className="py-2">
                                {isEditing ? (
                                  <Input
                                    ...
                                    onChange={(e) => setEditRowData({ ...editRowData, [header]: e.target.value })}
                                    className="h-8 text-sm border-[#E8E4DC]"
                                    placeholder="..."
                                  />
                                ) : (
                                  <span className="text-sm text-[#172030]">
                                    {String(row[header] || '')}
                                  </span>
                                )}
                              </TableCell>
                              ...
                              {isEditing ? (
                                ...
                                <div className="flex items-center justify-center gap-1">
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    className="h-7 w-7 p-0 text-[#172030]/40 hover:text-[#2A5141] hover:bg-[#F8F6F2]"
                                  ...
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    className="h-7 w-7 p-0 text-[#172030]/30 hover:text-red-600 hover:bg-red-50"
                    ...
                    <tfoot>
                      <TableRow className="bg-[#F8F6F2] border-t-2 border-[#E8E4DC]">
                        <TableCell colSpan={headers.length + 2} className="py-3 px-3 font-medium text-sm text-[#172030]">
                          Total : <span className="text-[#2A5141]">{editableData.length}</span> lignes à importer
                          {editableData.length !== totalRows && (
                            <span className="text-xs text-[#172030]/40 ml-2">
                              ({totalRows - editableData.length} supprimées)
                            </span>
                          )}
                        </TableCell>
                      </TableRow>
                ...
                <div className="grid grid-cols-4 gap-4">
                  <Card className="border-[#E8E4DC]">
                    <CardContent className="p-4 text-center">
                      <p className="text-xs text-[#172030]/40">Créées</p>
                      <p className="text-2xl font-bold text-green-600">{importResult.created}</p>
                    </CardContent>
                  </Card>
                  <Card className="border-[#E8E4DC]">
                    <CardContent className="p-4 text-center">
                      <p className="text-xs text-[#172030]/40">Mises à jour</p>
                      <p className="text-2xl font-bold text-blue-600">{importResult.updated}</p>
                    </CardContent>
                  </Card>
                  <Card className="border-[#E8E4DC]">
                    <CardContent className="p-4 text-center">
                      <p className="text-xs text-[#172030]/40">Ignorées</p>
                      <p className="text-2xl font-bold text-gray-400">{importResult.ignored}</p>
                    </CardContent>
                  </Card>
                  <Card className="border-[#E8E4DC]">
                    <CardContent className="p-4 text-center">
                      <p className="text-xs text-[#172030]/40">Erreurs</p>
...
const EmptyState = ({ 
  ...
  const getIcon = () => {
    switch(type) {
      case 'hr': return <Users className="h-12 w-12 text-[#172030]/20" />;
      case 'equipment': return <Monitor className="h-12 w-12 text-[#172030]/20" />;
      case 'app': return <Server className="h-12 w-12 text-[#172030]/20" />;
      case 'supplier': return <Handshake className="h-12 w-12 text-[#172030]/20" />;
      default: return <Building2 className="h-12 w-12 text-[#172030]/20" />;
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="w-20 h-20 rounded-full bg-[#F8F6F2] flex items-center justify-center mb-4">
        {getIcon()}
      </div>
      <h3 className="text-lg font-medium text-[#172030] mb-1">Aucune ressource</h3>
      <p className="text-sm text-[#172030]/40 mb-6">
        Aucune {getLabel()} trouvée dans le référentiel.
      </p>
...
const CMDBModule = () => {
  ...
  const loadResources = useCallback(async () => {
    ...
    try {
      ...
      console.log('📊 RH:', hrData?.length || 0, 'erreur:', hrError);

      const { data: equipData, error: equipError } = await supabase
        .from('ressources_equipements')
        .select('*');
      
      console.log('📊 Équipements:', equipData?.length || 0, 'erreur:', equipError);

      const { data: appData, error: appError } = await supabase
        .from('applications_it')
        .select('*');
      
      console.log('📊 Applications:', appData?.length || 0, 'erreur:', appError);

      const { data: suppData, error: suppError } = await supabase
        .from('fournisseurs')
        .select('*');
      
      console.log('📊 Fournisseurs:', suppData?.length || 0, 'erreur:', suppError);
      ...
      const getUsageCount = async (table: string, idColumn: string, ids: string[]) => {
        ...
        const counts: Record<string, { 
          ...
          processes: string[]; 
          processNames: string[];
          processCriticalities: Record<string, Criticality>;
        }> = {};
      ...
      const computeMaxCriticality = (counts: any, id: string) => {
        if (!counts[id] || Object.keys(counts[id].processCriticalities).length === 0) {
          return null;
        }
  ...
  // GESTIONNAIRE POUR LE DROPDOWN
  const handleUsageFilterChange = (value: UsageFilter) => {
  ...
  const deleteResource = async (resource: any) => {
    ...
    try {
      await supabase
        .from(tables[activeTab].link)
        .delete()
        .eq(tables[activeTab].idColumn, resource.id);

      await supabase
        .from(tables[activeTab].main)
        .delete()
        .eq('id', resource.id);
  ...
  const renderForm = (type: ResourceType, data: any, onChange: (d: any) => void) => {
    const forms = {
      hr: (
        <>
          <div className="space-y-1">
            <Label className="text-xs text-[#172030]/50 uppercase tracking-wider">Identité</Label>
          ...
          <div className="space-y-1">
            <Label className="text-xs text-[#172030]/50 uppercase tracking-wider">Contact</Label>
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div>
      ...
      equipment: (
        <>
          <div className="space-y-1">
            <Label className="text-xs text-[#172030]/50 uppercase tracking-wider">Informations générales</Label>
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div>
                <Label className="text-sm">Nom *</Label>
                <Input 
      ...
      app: (
        <>
          ...
          <div>
            <Label className="text-sm">Application alternative</Label>
            <Input 
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-sm">RTO (heures)</Label>
              <Input 
                type="number"
                min={0}
            ...
            <div>
              <Label className="text-sm">RPO (heures)</Label>
              <Input 
                type="number"
                min={0}
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-[#172030]" style={{ fontFamily: "'Playfair Display', serif" }}>
              Référentiel des ressources (CMDB)
            </h1>
          ...
          <div className="flex gap-2 shrink-0 flex-wrap">
            ...
            <Button
              ...
              className="gap-2 text-[#2A5141] border-[#2A5141] hover:bg-[#F8F6F2]"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
          ...
          <KpiCard
            ...
            color={unusedThreshold > 0.3 ? "#f97316" : "#172030"}
            onClick={() => handleKpiClick('unused')}
            isActive={usageFilter === 'unused'}
            isClickable={true}
            alertThreshold={unusedThreshold > 0.3 ? { 
              value: totalUnused, 
              message: `>30% des ressources (${Math.round(unusedThreshold * 100)}%)` 
            } : undefined}
          />
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Select value={usageFilter} onValueChange={handleUsageFilterChange}>
          <SelectTrigger className="w-full sm:w-[140px] border-[#E8E4DC]">
        ...
        <div className="flex items-center gap-2">
          <div className="text-xs text-[#172030]/40 whitespace-nowrap">
            {getFilteredResources(activeTab).length} / {resources[activeTab]?.length || 0}
          </div>
          <ToggleGroup type="single" value={viewMode} onValueChange={(v) => v && setViewMode(v as ViewMode)} className="border border-[#E8E4DC] rounded-lg p-0.5 bg-white">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as ResourceType)} className="w-full">
        <TabsList className="bg-transparent border-b border-[#E8E4DC] rounded-none p-0 h-auto gap-0 flex flex-wrap">
          {TABS.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <TabsTrigger 
                ...
                value={tab.id}
                className={cn(
                  "rounded-none border-b-2 border-transparent px-4 py-2.5 text-sm font-medium text-[#172030]/60 flex items-center gap-2 transition-all",
                  isActive && "border-[#2A5141] text-[#172030] bg-[#F0F5F0]"
                )}
              >
                <Icon className="h-4 w-4" />
                {tab.label}
                <Badge variant="outline" className="text-[10px] bg-[#F8F6F2] border-[#E8E4DC] text-[#172030]/60 ml-1">
                  {count}
                </Badge>
              </TabsTrigger>
            );
